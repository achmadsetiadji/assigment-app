...
if (!function_exists('load_file')) {
    ...
    {
        if (strpos($filepath, 'url_mqk_pendaftaran')) {
            if (env('APP_MQKN_ENV') === 'production') {
                if ($filepath == '{url_mqk_pendaftaran}/default') {
                    $url = 'https://mqkn.kemenag.go.id/masuk/img/img-backup.png';
                } else {
                    $url = str_replace('{url_mqk_pendaftaran}', 'https://mqkn.kemenag.go.id/masuk/storage', $filepath);
                }
                return $url;
            } else {
                if (strpos($filepath, '{url_mqk_pendaftaran}/default')) {
                    $url = 'https://mqkn.kemenag.go.id/masuk/img/img-backup.png';
                } else {
                    $url = str_replace('{url_mqk_pendaftaran}', 'https://dev.impstudio.id/mqk_nasional/storage', $filepath);
                }
                return $url;
            }
        }
