...
function resetForm(selector) {
    ...
    $(selector).find(".select2").trigger("change");
        ...
        if ($(`[name=${field}]`).attr("type") != "file") {
            if ($(`[name=${field}]`).hasClass("summernote")) {
                ...
                if (
                    ...
                    //select2Multiple
                    var select2Multiple = $(`[name="${field}[]"]`).hasClass(
                        "select2"
                    );
                    if (
            ...
            //select list
            if ($(`[name="${field}"]`).hasClass(`select-list-${field}`)) {
                $(`[name="${field}"]`).data("value", originalForm[field]);
            }
        } else {
...
/**
 * Confirm Alert Message WITH SWAL2
 *
 * @param   {string}  title
 * @param   {string}  desc
 * @param   {('success'| 'error' | 'warning' | 'info' | 'question') }  icon
 * @param {{ afterConfirm: void, afterCancel: void }} afterClick
 *
 * @return
 */
function confirmAlert(
    ...
    desc = "",
    icon = "warning",
    afterClick = { afterConfirm, afterCancel }
) {
    if (["success", "error", "warning", "info", "question"].includes(icon)) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                return afterClick.afterConfirm();
            } else {
        ...
        Swal.fire({
            title: title,
            text: desc,
            iconHtml: `<img src="${icon}" style="max-width: 60px;">`,
            showCancelButton: true,
            confirmButtonColor: getComputedStyle(
                document.documentElement
            ).getPropertyValue("--primary"),
